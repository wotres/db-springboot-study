...
* 인터페이스에 체크 예외코드 도입시 너무 많은 throws 가 필요하다.
  * 인터페이스가 예외를 포함해야하여 특정기술에 오염됨
  * 인터페이스 만드는 목적이 구현체를 쉽게 변경하기 위함인데 잘못된 방법
* 에러 코드를 통해 오류를 확인
* Transient 예외는 다시 시도시 성공 가능성이 있음
  * ex) 쿼리 타임아웃, 락
* NonTransient 예외는 SQL 문법오류 등으로 반복실행시 실패
* 스프링에서 제공하는 예외 사용 (특정 기술에 종속적이지않음)
* 스프링의 예외 추상화
  * sql-error-codes.xml 에서 다양한 SQL 의 ErrorCode를 명시
* JdbcTemplate 를 이용하여 JDBC 의 반복 문제 해결 가능
